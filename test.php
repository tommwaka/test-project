...
//$presId=mysql_fetch_array($getPresid);
	...
	$sqlP=mysql_query("INSERT INTO prescription(customer_id,customer_name,age,sex,postal_address,invoice_id,phone)
				VALUES('{$c_id}','{$cname}','{$age}','{$sex}','{$postal}','{$invoiceNo}','{$phone}')  ");
...
class MYPDF extends TCPDF {
	...
	public function ColoredTable($header,$data) {
		...
		foreach($data as $row) {
			...
			//$this->Cell($w[4], 4, number_format($row[4],2.0), 'LR', 0, 'R', $fill);
			//$this->Cell($w[5], 4, number_format($row[5],2.0), 'LR', 0, 'R', $fill);
			$this->Ln();
			$fill=!$fill;
		}
...
?>
 <div class="form-row" align="left">
		 ...
		 <?php $getpayType=mysql_query("SELECT drug_name FROM stock"); ?>
